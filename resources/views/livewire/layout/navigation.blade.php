...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-200 shadow-sm">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:space-x-4">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-gray-600 hover:text-indigo-600 focus:outline-none transition">
                            <span x-data="{{ json_encode(['name' => auth()->user()->name]) }}"
                                  x-text="name"
                                  x-on:profile-updated.window="name = $event.detail.name">
                            </span>
                            <svg class="ml-2 h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden bg-white border-t border-gray-200">
        ...
        <div class="pt-4 pb-1 border-t border-gray-100">
            <div class="px-4">
                <div class="font-semibold text-gray-800" x-data="{{ json_encode(['name' => auth()->user()->name]) }}"
